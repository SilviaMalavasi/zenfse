...
if (!function_exists('zenfse_setup')) {
  function zenfse_setup()
  {
    ...
    if (!is_admin() && !is_login()) {
      wp_enqueue_style('zenfse-reset', get_stylesheet_directory_uri() . '/dist/reset.css', array(), filemtime(get_stylesheet_directory() . '/dist/reset.css'), false);
    }
    ...
    // Enqueue front-only style

    if (!is_admin() && !is_login()) {
      wp_enqueue_style('zenfse-style-front-only', get_stylesheet_directory_uri() . '/dist/style-front-only.css', array(), filemtime(get_stylesheet_directory() . '/dist/style-front-only.css'), false);
    }

    // Enqueue script

    wp_enqueue_script('zenfse-script', get_stylesheet_directory_uri() . '/dist/site.js', array('jquery'), filemtime(get_stylesheet_directory() . '/dist/site.js'), true);
  }
}
...
{
  wp_enqueue_style('zenfse-admin-style', get_template_directory_uri() . '/dist/style-back-only.css');
}
...
function zenfse_core_blocks_scripts()
{
  ...
  wp_enqueue_script('zenfse-enqueue-image-style-script', get_stylesheet_directory_uri() . '/blocks/build-core-blocks/image-style.js', array('jquery'), filemtime(get_stylesheet_directory() . '/blocks/build-core-blocks/image-style.js'), true);
...
function zenfse_enqueue_frontend_core_image()
{
  if (!is_admin()) {
    ...
    if (has_block('core/image', $id)) {
      wp_enqueue_script('zenfse-core-blocks-image-frontend', get_stylesheet_directory_uri() . '/blocks/build-core-blocks/image-dimension-frontend.js', array('jquery'), filemtime(get_stylesheet_directory() . '/blocks/build-core-blocks/image-dimension-frontend.js'), true);
